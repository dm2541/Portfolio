<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="ko" xml:lang="ko" xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta name="google-site-verification"
	content="NqB0BDAEWJTvAPCCxzrckJYnS7-xJILFU40FvSmh5S8">
<title>마초맨 - 스트릿패션, 패션잡지, 멀티샵, 중고장터</title>
<link type="text/css" rel="stylesheet" charset="utf-8"
	href="//static.musinsa.com/mfile_outsrc/css/common/normalizer.css?v=9.21384">
<link type="text/css" rel="stylesheet" charset="utf-8"
	href="//static.musinsa.com/mfile_outsrc/css/common/header.css?v=9.21384">
<link type="text/css" rel="stylesheet" href="css/style_nav.css">
</head>
<body>
	<div class="top-column column">
		<!--헤더 header start-->
		<div class="header clearfix">
			<div class="wrapper">
					<h1><a href="admin.jsp">MACHOMANZ STORE</a></h1>
			</div>
			<div class="userMenu-wrapper toggleBox">
				<ul class="userMenu-list-notLogin userMenu-list clearfix">
					<li class="listItem loginBtn"><a href="/logout.do">관리자님 환영합니다.</a></li>
					<li class="listItem"><a href="/logout.do" title="로그아웃"><span>로그아웃</span></a></li>
					<li class="listItem"><a href="/productList.do" title="제품목록"><span>제품목록</span></a></li>
					<li class="listItem"><a href="/productInsert.jsp" title="제품등록"><span>제품등록</span></a></li>
					<li class="listItem"><a href="/customerList.do" title="회원목록"><span>회원목록</span></a></li>
					<li class="listItem"><span>&nbsp;&nbsp;EVENT. 신규 가입 시,
							10% 할인 이벤트 진행중</span></li>
				</ul>
			</div>
		</div>
	</div>
</body>
</html>