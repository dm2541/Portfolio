...
<script>
	...
	function menu(){
		...
		$('#product_choice').on('change', function(){
			...
			var menuInfo = '<p>' + product_name + '&nbsp;&nbsp;&nbsp;' + product_price + '</p>';
			productList.push(product_name);
			$('#menu_div').append(menuInfo);

			priceAll += product_price;
			var account = '<p> 총 합계금액 &nbsp;&nbsp;&nbsp;' + priceAll +
			'<button type="button" id="order_btn" onclick="location.herf="insertOrderList.do"" style="margin-left:10px;">주문하기</button>' +
			'</p>';
			$('#menu_account').html(account);
			orderListInsert(productList);
		})
	}
	
 	/* function orderListInsert(productList) {
		$('#order_btn').on('click', function() {
			$.ajax({
				url: '/myapp/orderListInsert.do',
				type: 'POST',
				data: {
					user_id: '${user.user_id}',
